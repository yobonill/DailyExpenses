...
import { CASH_ACCOUNT_ID, getActiveBankAccounts, getMoneyAccountSpendableBalance, hasUnifiedSavingsAccounts, moneyAccountLabel } from "../../lib/moneyLedger";
...
function FundForm({ data, fund, onSave, onClose }: {
  ...
  const unified = hasUnifiedSavingsAccounts(data);
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (unified && !moneyAccountId && (!fund || getFundBalance(data, fund.id) > 0 || (initialBalanceMinor || 0) > 0)) return setError("Selecciona dónde está guardado este ahorro.");
    try {
      await onSave({
        name,
        currency,
  ...
  return <Modal title={fund ? "Editar fondo" : "Nuevo fondo"} onClose={onClose}>
    <form className="form-grid" onSubmit={submit}>
      ...
      <label className="field"><span>¿Dónde está guardado?{unified ? "" : " (opcional)"}</span><select value={moneyAccountId} onChange={(event) => setMoneyAccountId(event.target.value)}><option value="">Sin especificar</option>{physicalAccounts.map((account) => <option value={account.id} key={account.id}>{moneyAccountLabel(account.id, data)} · Disponible sin apartar {formatCurrency(getMoneyAccountSpendableBalance(data, account.id), account.currency)}</option>)}</select><small className="field-help">El fondo representa una parte apartada del saldo total de esta cuenta; no se suma como dinero adicional.</small></label>
...
function TransactionModal({ data, fund, onAdd, onTransfer, onClose }: {
  ...
  return <Modal title={`Movimiento · ${fund.name}`} onClose={onClose}><form className="form-grid" onSubmit={submit}>
    {hasUnifiedSavingsAccounts(data) && <p className="privacy-note">Apartar o liberar dinero cambia cuánto está reservado dentro de la cuenta, pero no cambia su saldo total.</p>}
    <label className="field"><span>Tipo</span><select value={type} onChange={(event) => setType(event.target.value as typeof type)}><option value="deposit">Apartar dinero</option><option value="withdrawal">Liberar dinero del fondo</option><option value="correction">Corregir balance del fondo</option><option value="transfer">Transferir a otro fondo</option></select></label>
