...
import { CASH_ACCOUNT_ID, getActiveBankAccounts, getMoneyAccountBalance, moneyAccountLabel } from "../../lib/moneyLedger";
import type { Currency, FinancialData, IncomeOccurrence, IncomeTemplate, MoneyAccountId } from "../../models/finance";
...
function IncomeFormModal({ template, onSaveTemplate, onCreateOneTime, onClose }: {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (mode === "oneTime") await onCreateOneTime({ name, expectedAmountMinor, currency, expectedDate: date, notes, exportExpectedWhenPending: true });
      else await onSaveTemplate({ name, expectedAmountMinor, currency, incomeType, dueRule: lastDay ? { kind: "lastDay" } : { kind: "day", day: dueDay }, active, notes, exportExpectedWhenPending: true }, template?.id);
      onClose();
...
function ReceiveModal({ data, occurrence, onConfirm, onClose }: { data: FinancialData; occurrence: IncomeOccurrence; onConfirm: (amount: number, date: string, accountId?: MoneyAccountId) => Promise<void>; onClose: () => void }) {
  ...
  const destinationAccounts = [...getActiveBankAccounts(data), ...(data.moneyAccounts[CASH_ACCOUNT_ID]?.active ? [data.moneyAccounts[CASH_ACCOUNT_ID]] : [])];
  const [accountId, setAccountId] = useState<MoneyAccountId>(destinationAccounts[0]?.id || "");
  const submit = async (event: FormEvent) => { event.preventDefault(); const value = parseMoneyToCents(amount); if (!value) return setError("Monto inválido."); if (occurrence.currency === "DOP" && !accountId) return setError("Selecciona la cuenta donde recibiste el ingreso."); setSaving(true); try { await onConfirm(value, date, occurrence.currency === "DOP" ? accountId : undefined); onClose(); } catch (reason) { setError(reason instanceof Error ? reason.message : "No se pudo registrar el ingreso."); } finally { setSaving(false); } };
  return <Modal title={`Marcar recibido · ${occurrence.name}`} onClose={onClose}><form className="form-grid" onSubmit={submit}><div className="form-summary"><span>Esperado</span><strong>{formatCurrency(occurrence.expectedAmountMinor, occurrence.currency)}</strong></div><MoneyField label="Monto recibido" value={amount} onChange={setAmount} currency={occurrence.currency} /><label className="field"><span>Fecha recibida</span><input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label>{occurrence.currency === "DOP" && <label className="field"><span>¿Dónde entró el dinero?</span><select value={accountId} onChange={(event) => setAccountId(event.target.value)}><option value="">Seleccionar banco y cuenta</option>{destinationAccounts.map((account) => <option value={account.id} key={account.id}>{moneyAccountLabel(account.id, data)} · {formatCurrency(getMoneyAccountBalance(data, account.id), "DOP")}</option>)}</select><small className="field-help">Este ingreso aumentará únicamente el saldo seleccionado.</small></label>}{error && <p className="form-error">{error}</p>}<div className="modal-actions"><button type="button" className="button button-secondary" onClick={onClose}>Cancelar</button><button className="button button-primary" disabled={saving}>Confirmar</button></div></form></Modal>;
}
...
export function IncomeView({ data, onSaveTemplate, onCreateOneTime, onReceive, onReopen }: {
  ...
  onSaveTemplate: (input: IncomeTemplateInput, id?: string) => Promise<void>;
  onCreateOneTime: (input: OneTimeIncomeInput) => Promise<void>;
  onReceive: (id: string, amount: number, date: string, accountId?: MoneyAccountId) => Promise<void>;
  ...
  return <section className="finance-page"><PageHeading eyebrow="Dinero que entra" title="Ingresos" action={<button className="button button-primary heading-action" type="button" onClick={() => setForm("new")}>＋ Agregar</button>} /><PeriodSelector monthKey={monthKey} onMonthChange={setMonthKey} quincena={quincena} onQuincenaChange={setQuincena} />
    ...
    {form && <IncomeFormModal template={form === "new" ? undefined : form} onSaveTemplate={onSaveTemplate} onCreateOneTime={onCreateOneTime} onClose={() => setForm(null)} />}{receiving && <ReceiveModal data={data} occurrence={receiving} onConfirm={(amount, date, accountId) => onReceive(receiving.id, amount, date, accountId)} onClose={() => setReceiving(null)} />}
  </section>;
}
