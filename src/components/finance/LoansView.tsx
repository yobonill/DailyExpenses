...
function LoanForm({ data, loan, onSave, onClose }: {
  ...
  return <Modal title={loan ? "Editar préstamo" : "Nuevo préstamo"} onClose={onClose} confirmClose>
    <form className="form-grid" onSubmit={submit}>
      ...
      <label className="field"><span>Banco</span><select value={bankId} onChange={(event) => setBankId(event.target.value)}><option value="">Sin vincular</option>{banks.map((bank) => <option value={bank.id} key={bank.id}>{bank.name}</option>)}</select><small className="field-help">Los bancos se administran en Más → Bancos y efectivo.</small></label>
